...
fn dies_bad_type() -> TestResult {
    let expected =
        "error: invalid value 'x' for '--type <ENTRY_TYPES>...': [possible values: d, f, l]";
    Command::cargo_bin(PRG)?
        .args(&["--type", "x"])
        .assert()
        .failure()
...
#[test]
fn path1_maxdepth1() -> TestResult {
    run(
        &["tests/inputs", "--max-depth=1"],
        "tests/expected/path1_maxdepth1.txt",
    )
}
...
#[test]
fn path_a_maxdepth1() -> TestResult {
    run(
        &["tests/inputs/a/", "--max-depth=1"],
        "tests/expected/path_a_maxdepth1.txt",
    )
}

#[test]
fn path_a_maxdepth2_mindepth1() -> TestResult {
    run(
        &["tests/inputs/a/", "--max-depth=2", "--min-depth=1"],
        "tests/expected/path_a_maxdepth2_mindepth1.txt",
    )
}
